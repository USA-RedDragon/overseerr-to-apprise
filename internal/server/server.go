...
func NewServer(config *config.Config, metrics *metrics.Metrics) *Server {
	...
	applyMiddleware(r, config, "api", metrics)
	...
	if config.HTTP.Metrics.Enabled {
		...
		applyMiddleware(metricsRouter, config, "metrics", metrics)
